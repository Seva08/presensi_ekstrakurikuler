...
error_log("Dashboard Siswa Access - Time: " . date('Y-m-d H:i:s') . " | Role: " . ($_SESSION['role'] ?? 'unset') . " | User ID: " . ($_SESSION['user_id'] ?? 'unset') . " | Username: " . ($_SESSION['username'] ?? 'unset'));
...
if (empty($_SESSION) || !isset($_SESSION['role']) || $_SESSION['role'] !== 'siswa' || !isset($_SESSION['user_id']) || empty($_SESSION['user_id']) || !isset($_SESSION['username']) || empty($_SESSION['username'])) {
    ...
    try {
        ...
        error_log("Executing query with User ID: " . $_SESSION['user_id'] . ", Date: " . $current_date);
        $stmt = $pdo->prepare("
        ...
        ");
        $stmt->execute([$_SESSION['user_id'], $current_date, $_SESSION['user_id']]);
        $extracurriculars = $stmt->fetchAll(PDO::FETCH_ASSOC);
        error_log("Query result: " . json_encode($extracurriculars));
    ...
    <div class="container my-4">
        ...
        <div class="card-main">
            ...
            <div class="row mb-4">
                ...
                <div class="col-md-6">
                    <a href="index.php?page=presensi_siswa" class="btn btn-secondary feature-btn">
